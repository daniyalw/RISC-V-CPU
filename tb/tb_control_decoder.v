module tb_control_decoder;
    ...
    control_decoder uut (.opcode(opcode), .funct3(funct3), .funct7(funct7), .alu_op(alu_op), .alu_src(alu_src), .reg_write(reg_write), .invalid_instruction(invalid_instruction));
    ...
    task check;
        ...
        begin
            ...
            if ((alu_op !== expected_op) || (alu_src !== expected_src) || (reg_write !== expected_regwrite) || (invalid_instruction !== expected_invalid)) begin
                $display("Error: test %0d | opcode = %b, funct3 = %b, funct7 = %b | alu_op = %b (expected = %b), alu_src = %b (expected = %b), reg_write = %b (expected = %b), invalid_instruction = %b (expected = %b)", num_tasks, opcode, funct3, funct7, alu_op, expected_op, alu_src, expected_src, reg_write, expected_regwrite, invalid_instruction, expected_invalid);
