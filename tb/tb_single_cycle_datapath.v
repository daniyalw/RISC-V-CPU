module tb_single_cycle_datapath;
    ...
    single_cycle_datapath uut (.instruction(instruction), .clk(clk), .reset(reset), .alu_result(alu_result));
    ...
    task check_task;
        ...
        begin
            ...
            if (alu_result !== expected_result) begin
                $display("Error: instruction = %h | ALU result = %h (expected = %h), register x%0d value = %h (expected = %h)", instruction, alu_result, expected_result, port1, uut.rs1_data, expected_result);
