module single_cycle_datapath (input [31:0] instruction, input clk, reset, input [31:0] pc, output [31:0] alu_result, output invalid_instruction, output branch_taken, output [31:0] branch_target);
    wire [6:0] opcode;
    wire [2:0] funct3;
    wire [6:0] funct7;
    wire [2:0] alu_op;
    ...
    control_decoder cd (.opcode(opcode), .funct3(funct3), .funct7(funct7), .alu_op(alu_op), .branch(branch), .branch_type(branch_type), .alu_src(alu_src), .reg_write(reg_write), .invalid_instruction(invalid_instruction), .mem_read(mem_read), .mem_write(mem_write), .mem_to_reg(mem_to_reg), .jal_enable(jal_enable), .jalr_enable(jalr_enable));
